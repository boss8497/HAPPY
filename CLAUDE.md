  ...
  **핵심 설계**
  ...
  - **SafeArea**: 전용 `SafeArea` Screen을 최상위 레이어에 열어 입력 차단 (`ShowSafeAreaAsync`/`HideSafeAreaAsync`). 열릴 때마다 `defaultTimer`(기본 5초, `SafeAreaOption.Time`으로 Show 시점에 오버라이드 가능) 워치독이 돌아서 닫기 호출이 누락돼도 자기 자신을 강제 `CloseAsync`로 자동 복구
  ...
  **지원:** 연산자 `+`, `-`, `*`, `/`, 단항 `-` / 함수 `Pow`, `Log`, `Min`, `Max`
  ...
  ### Tutorial — 튜토리얼 오케스트레이션 + Focus(스포트라이트)/Narration(대사창)
  - 위치: `Assets/Script/GamePlay/Tutorial/`(오케스트레이션 `TutorialService`/등록/데이터), `Assets/Script/GamePlay/Service/`(`FocusService`/`NarrationService`), `Assets/Script/GUI/Screen/Tutorial/`(`TutorialFocusScreen`/`TutorialNarrationScreen`), `Assets/Script/GUI/ScreenOption/`(`FocusOption`/`NarrationOption`)
  - `ITutorialService.StartTutorial(uid)` → `TutorialService`가 `TutorialInfo.sets`(GameInfo, `GuideBase[]`)를 순서대로 진행, 가이드 타입별로 `FocusService`/`NarrationService`에 분기하고 각 스텝의 완료 콜백을 실제로 기다린 뒤 다음 스텝으로 넘어감(타입이 바뀌면 이전 화면을 강제 종료)
  - 씬의 UI 요소에 `FocusComponent`를 붙이면 `Start()` 시점에 `IFocusService.RegisterFocusData()`로 자동 등록(`OnDestroy()`에서 자동 해제)
  - `FocusService.StartAsync(GuideBase guide, ...)`(`IFocusService`/`INarrationService` 공통 `ITutorial` 인터페이스 — 이름이 같은 인터페이스가 여러 계층에 있어 헷갈리기 쉬움, ARCHITECTURE.md 참고) — `FocusGuide`를 넘기면 id/focusGuid로 등록된 대상을 찾아(최대 100회 재시도 — UI가 아직 로딩 중인 타이밍 문제 흡수) `TutorialFocusScreen`을 열고 스포트라이트 + 가이드 텍스트 표시
  ...
  - `NarrationGuide`는 클릭 대상 없이 아이콘+텍스트만 보여주고 다음 버튼을 누르면 완료 — `NarrationService`가 대상 조회 없이 곧바로 `TutorialNarrationScreen`을 열고 닫음
